...
export async function PATCH(
  ...
  try {
    ...
    const { success: rateOk, retryAfter } = await checkRateLimitAsync(`availability-patch:${user.id}`, {
      windowMs: 60_000,
      max: 60,
    })
    if (!rateOk) {
...
export async function DELETE(
  ...
  try {
    ...
    const { success: rateOk, retryAfter } = await checkRateLimitAsync(`availability-delete:${user.id}`, {
      windowMs: 60_000,
      max: 60,
    })
    if (!rateOk) {
