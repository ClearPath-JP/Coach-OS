 ...
 *   can serve many coaches, each with their own folder ID saved in Korva.
 */
export async function POST(request: Request) {
  try {
    ...
    if (isN8n) {
      ...
      if (!resolved) {
        return NextResponse.json(
          { error: 'No workspace registered for this Drive folder — coach must save folder ID in Korva first' },
          { status: 404 }
        )
      }
